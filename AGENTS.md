...
`deploy-ss` is the only non-git skill in the table. It turns raw app UI captures (in `raw-screenshots/`, gitignored — captured per-platform from the iOS simulator and Android emulator) into the polished store assets: the `screenshots/` set used in the README and App Store listing (6 phone shots at 1284 × 2778, 6 tablet shots at 2048 × 2732 for the App Store iPad slot — Play Store tablet listings would need a separate render pass at Google's 7" / 10" dimensions, plus 1 feature graphic at 1024 × 500), and the `play-store/` set (6 framed phone shots at 1284 × 2778) for Google Play. The current lineup is Markets · **Portfolio** · Charts · Depth · News · Favorites. Shots use an iPhone-styled bezel but carry each platform's own status bar (iOS captures for the App Store set, Android captures for Play Store); Compose Multiplatform means the underlying UI is identical across both. For the Portfolio screen, demo wallets are seeded from the Hyperliquid public leaderboard. The skill bundles its own Python renderer, Inter fonts, and design references, and refreshes the README screenshot tables on every run — see `.claude/skills/deploy-ss/SKILL.md`.
