...
`deploy-ss` is the only non-git skill in the table. It turns raw app UI captures (in `raw-screenshots/`, gitignored — captured per-platform from the iOS simulator and Android emulator) into the polished store assets: the `screenshots/` set used in the README and App Store listing (8 phone shots at 1284 × 2778, 8 tablet marketing frames at 2064 × 2752 in `screenshots/ipad/` for the App Store 13" iPad Pro Display slot — Play Store tablet listings would need a separate render pass at Google's 7" / 10" dimensions — plus 1 feature graphic at 1024 × 500), and the `play-store/` set (8 framed phone shots at 1284 × 2778) for Google Play. The current lineup is Markets · **Portfolio** · **AI Insights** · **Allocation** (donut) · Charts · Depth · News · Favorites. Shots use an iPhone-styled bezel but carry each platform's own status bar (iOS captures for the App Store set, Android captures for Play Store); Compose Multiplatform means the underlying UI is identical across both. For the Portfolio screen, demo wallets are seeded from the Hyperliquid public leaderboard; the AI Insights card needs no setup (the provider is keyless) — just allow it a moment to generate before capturing. The skill bundles its own Python renderer, Inter fonts, and design references, and refreshes the README screenshot tables on every run — see `.claude/skills/deploy-ss/SKILL.md`.
