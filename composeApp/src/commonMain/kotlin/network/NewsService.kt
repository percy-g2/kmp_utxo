...
class NewsService {
    ...
    /** In-memory cache (max 10 keys, LRU) — plain [MutableMap] for K/N (LinkedHashMap is not open on native). */
    private val cache = mutableMapOf<String, CachedNews>()
    private val cacheLruKeys = mutableListOf<String>()
    ...
    suspend fun fetchNewsFromProvider(
        ...
        val cacheKey = "$coinSymbol|${provider.id}"
        cacheMutex.withLock {
            val now =
            ...
            if (hit != null && now - hit.timestamp < CACHE_TTL_MS) {
                cacheLruKeys.remove(cacheKey)
                cacheLruKeys.add(cacheKey)
                return hit.news
            }
        }
        return try {
            ...
            if (news != null) {
                ...
                val deduped = news.distinctBy { it.link }
                cacheMutex.withLock {
                    putCacheEntry(
                        cacheKey,
                        CachedNews(
    ...
    private fun putCacheEntry(
        ...
        cache.remove(key)
        cache[key] = value
        cacheLruKeys.remove(key)
        cacheLruKeys.add(key)
        while (cacheLruKeys.size > MAX_CACHE_ENTRIES) {
            val evict = cacheLruKeys.removeAt(0)
            cache.remove(evict)
        }
    }
    ...
    private companion object {
        ...
        const val MAX_CACHE_ENTRIES = 10
    }
    ...
    private fun parseRSSFeed(
        ...
        if (baseSymbol.isEmpty() || baseSymbol == coinSymbol.uppercase()) {
            ...
            val knownCoins =
                listOf("BTC", "ETH", "BNB", "ADA", "SOL", "XRP", "DOT", "DOGE", "MATIC", "AVAX", "LINK", "UNI", "LTC", "ATOM", "ETC", "XLM", "ALGO", "VET", "FIL", "TRX", "EOS", "AAVE", "MKR", "COMP", "YFI", "SUSHI", "SNX", "CRV", "1INCH", "BAL", "ZRX", "ENJ", "MANA", "SAND", "AXS", "GALA", "CHZ", "FLOW", "THETA", "HBAR", "NEAR", "FTM", "ICP", "APT", "ARB", "OP", "SUI", "SEI", "TIA", "INJ", "RUNE", "KAVA", "WAVES", "ZEC", "DASH", "XMR")
        ...
        items.forEach { match ->
            ...
            if (!shouldInclude) {
                when (upperBaseSymbol) {
                    "BTC" ->
                        shouldInclude = searchText.contains("BITCOIN", ignoreCase = true) ||
                            searchText.contains(" BTC ", ignoreCase = true) ||
                            searchText.startsWith("BTC", ignoreCase = true)
                    "ETH" ->
                        shouldInclude = searchText.contains("ETHEREUM", ignoreCase = true) ||
                            searchText.contains(" ETH ", ignoreCase = true) ||
                            searchText.startsWith("ETH", ignoreCase = true)
                    "BNB" ->
                        shouldInclude = searchText.contains("BINANCE", ignoreCase = true) ||
                            searchText.contains(" BNB ", ignoreCase = true)
                    "ADA" ->
                        shouldInclude = searchText.contains("CARDANO", ignoreCase = true) ||
                            searchText.contains(" ADA ", ignoreCase = true)
                    "SOL" ->
                        shouldInclude = searchText.contains("SOLANA", ignoreCase = true) ||
                            searchText.contains(" SOL ", ignoreCase = true)
                    "XRP" ->
                        shouldInclude = searchText.contains("RIPPLE", ignoreCase = true) ||
                            searchText.contains(" XRP ", ignoreCase = true)
                    "DOT" ->
                        shouldInclude = searchText.contains("POLKADOT", ignoreCase = true) ||
                            searchText.contains(" DOT ", ignoreCase = true)
                    "DOGE" ->
                        shouldInclude = searchText.contains("DOGECOIN", ignoreCase = true) ||
                            searchText.contains("DOGE", ignoreCase = true)
                    "MATIC" ->
                        shouldInclude = searchText.contains("POLYGON", ignoreCase = true) ||
                            searchText.contains("MATIC", ignoreCase = true)
                    "AVAX" ->
                        shouldInclude = searchText.contains("AVALANCHE", ignoreCase = true) ||
