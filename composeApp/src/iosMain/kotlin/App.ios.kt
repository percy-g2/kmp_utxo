...
actual fun getAlertsKStore(): KStore<List<PriceAlert>> {
    val paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, true)
    return storeOf(
        file = Path("${paths.firstOrNull() as? String}/price_alerts.json"),
        default = emptyList(),
    )
}
...
actual object PriceAlertPlatformController {
    actual fun onEnabledAlertsChanged(
        ...
        if (enabledCount > 0) {
            userDefaults.setObject("1", forKey = "UTXO_PriceAlertsOn")
            if (enabledAlertsJson.isNotEmpty()) {
                userDefaults.setObject(enabledAlertsJson, forKey = "UTXO_PriceAlertsJson")
            }
        } else {
            userDefaults.removeObjectForKey("UTXO_PriceAlertsOn")
            userDefaults.removeObjectForKey("UTXO_PriceAlertsJson")
        }
